...
def plot_violin_cell_type_along_NT_score(
    ...
    **kwargs,
    ...
    data_df :
        pd.DataFrame, the data for visualization.
    cell_types :
        List[str], the cell types.
    cell_type_coding :
        np.ndarray, the cell type coding matrix.  #cell/spots x #cell_types
    output_file_path :
        Optional[Union[str, Path]], the output directory.
    kwargs :
        Optional, the additional arguments for visualization using seaborn.violinplot.
    ...
    if (n_cell_type := len(cell_types)) > 100:
        warning(
            "More than 100 cell types detected; skipping violin plot to avoid long runtime."
        )
        return None
...
def plot_violin_cell_type_along_NT_score_from_anadata(
    ana_data: AnaData, **kwargs
) -> Optional[Tuple[plt.Figure, plt.Axes]]:
    """Plot violinplot cell type composition along NT score.
    ...
    if ana_data.options.reverse:
        data_df["Cell_NTScore"] = 1 - data_df["Cell_NTScore"]

    cell_types = ana_data.meta_data_df["Cell_Type"].cat.categories.to_list()

    return plot_violin_cell_type_along_NT_score(
        data_df=data_df,
...
def plot_kde_cell_type_along_NT_score(
    data_df: pd.DataFrame, cell_type_coding: np.ndarray, output_file_path: Optional[Union[str, Path]] = None, **kwargs
) -> Optional[Tuple[plt.Figure, plt.Axes]]:
    ...
    output_file_path :
        Optional[Union[str, Path]], the output directory.
    kwargs :
        Optional, the additional arguments for visualization using seaborn.kdeplot.
...
def plot_kde_cell_type_along_NT_score_from_anadata(
    ana_data: AnaData, **kwargs
) -> Optional[Tuple[plt.Figure, plt.Axes]]:
    """Plot kdeplot cell type composition along NT score.
    ...
    return plot_kde_cell_type_along_NT_score(
        data_df=data_df, cell_type_coding=ana_data.cell_type_coding, output_file_path=ana_data.options.output, **kwargs
    )
...
def plot_hist_cell_type_along_NT_score(
    ...
    **kwargs,
    ...
    data_df :
        pd.DataFrame, the data for visualization.
    cell_types :
        List[str], the cell types, used for hue order of histogram.
    cell_type_coding :
        np.ndarray, the cell type coding matrix.  #cell/spots x #cell_types
    output_file_path :
        Optional[Union[str, Path]], the output directory.
    kwargs :
        Optional, the additional arguments for visualization using seaborn.histplot.
...
def plot_hist_cell_type_along_NT_score_from_anadata(
    ana_data: AnaData, **kwargs
) -> Optional[Tuple[plt.Figure, plt.Axes]]:
    """Plot histogram of cell type composition along NT score.
    ...
    return plot_hist_cell_type_along_NT_score(
        data_df=data_df,
        cell_types=cell_types,
        cell_type_coding=ana_data.cell_type_coding,
        output_file_path=ana_data.options.output,
...
def plot_cell_type_along_NT_score(ana_data: AnaData) -> None:
    ...
    if ana_data.options.spatial_res == "cell":  # do not support low resolution data yet
        plot_violin_cell_type_along_NT_score(
            data_df=data_df,
            cell_types=cell_types,
            cell_type_coding=ana_data.cell_type_coding,
            output_file_path=ana_data.options.output,
        )
        plot_kde_cell_type_along_NT_score(
            data_df=data_df, cell_type_coding=ana_data.cell_type_coding, output_file_path=ana_data.options.output
        )
        plot_hist_cell_type_along_NT_score(
            data_df=data_df,
            cell_types=cell_types,
            cell_type_coding=ana_data.cell_type_coding,
            output_file_path=ana_data.options.output,
