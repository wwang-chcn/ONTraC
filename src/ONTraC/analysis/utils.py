...
def saptial_figsize(sample_df, scaling_factor: Union[int, float] = 1) -> Tuple[int, int]:
    ...
    scale_factor :
        float, controls plot size; larger values produce larger figures.
    ...
    fig_width = x_span / 2_000 * scaling_factor * np.sqrt(points_density) + 0.5  # Adding 2 for colorbar space
    fig_height = y_span / 2_000 * scaling_factor * np.sqrt(points_density) + 0.2  # Adding 1.5 for title space
...
def validate_cell_type_palette(
    cell_types: List[str], palette: Union[List[str], Dict[str, str], None] = None
) -> Dict[str, str]:
    """
...
def cal_niche_level_raw_cell_type_counts_from_anadata(ana_data: AnaData) -> pd.DataFrame:
    ...
    cell_label_one_hot = np.zeros(
        shape=(ana_data.meta_data_df.shape[0], len(ana_data.meta_data_df["Cell_Type"].cat.categories))
    )
    cell_label_one_hot[np.arange(ana_data.meta_data_df.shape[0]), cell_type] = 1  # N(#cells) x F(#cell_types)
    cell_label_one_hot_df = pd.DataFrame(
        data=cell_label_one_hot,
    ...
    for sample in ana_data.meta_data_df["Sample"].unique():
        ...
        neighborIndices_matrix = np.loadtxt(
            f"{ana_data.options.NN_dir}/{sample}_NeighborIndicesMatrix.csv.gz", delimiter=","
        ).astype(int)
