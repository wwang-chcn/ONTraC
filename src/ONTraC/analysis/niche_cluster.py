...
def cal_nc_feat_from_anadata(ana_data: AnaData, increment=True) -> Optional[pd.DataFrame]:
    ...
    if increment:
        ...
        for sample in ana_data.rel_params["Data"]:
            ...
            cell_type_composition_df.columns = ana_data.cell_type_codes.loc[
        ...
        # Full calculation
        return cal_nc_feat(
            nc_assign=ana_data.niche_level_niche_cluster_assign,
...
def cal_nc_feat_similarity(nc_feat: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        data=1 - distance.cdist(XA=nc_feat, XB=nc_feat, metric="cosine"), index=nc_feat.index, columns=nc_feat.index
    )
...
def plot_niche_cluster_connectivity(
    ...
    reverse :
        bool, whether to reverse the color.
    figsize_scale :
        float, figure-size scale; if None, inferred from the number of niche clusters.
    output_file_path :
    ...
    # preprocessing
    # STEP: Set diagonal to 0
    np.fill_diagonal(niche_cluster_connectivity, 0)
    # STEP: Set figure size
    if figsize_scale is None:
        figsize_scale = (
            1.0 if niche_cluster_connectivity.shape[0] < 10 else 1.5 * np.log10(niche_cluster_connectivity.shape[0])
        )
    ...
    # Create a figure
    # SECTION: Figure width configuration
    figwidths = [6]
    if niche_cluster_size is not None:
        figwidths.append(1.5)  # type: ignore
    if niche_cluster_score is not None:
        figwidths.append(0.5)  # type: ignore
    figwidths.append(0.5)  # type: ignore
    ...
    # Draw the graph
    nx.draw_networkx_nodes(G=G, pos=pos, node_color=nc_colors, node_size=node_size, ax=graph_ax)  # type: ignore
    ...
    if niche_cluster_size is not None:
        ...
        # STEP: Draw node-size legend
        handles = [
            plt.Line2D(  # type: ignore
                [0], [0], marker="o", color="w", markerfacecolor="gray", markersize=np.sqrt(size), label=label
            )
            for size, label in zip(sizes, labels)
        ]
        legend = node_size_ax.legend(
            handles=handles, title="Niche cluster size", loc="center", frameon=False, bbox_to_anchor=(0.6, 0.6)
        )
        # STEP: Rotate legend title and adjust position
...
def plot_niche_cluster_connectivity_from_anadata(ana_data: AnaData) -> Optional[Tuple[plt.Figure, List[plt.Axes]]]:
    ...
    output_file_path = (
        f"{ana_data.options.output}/niche_cluster_connectivity.pdf" if ana_data.options.output is not None else None
    )
...
def plot_niche_cluster_connectivity_bysample_from_anadata(ana_data: AnaData) -> None:
    ...
    for sample in ana_data.meta_data_df["Sample"].unique():
        ...
        if ana_data.cell_level_niche_cluster_assign is None:
            ...
            niche_cluster_size = ana_data.cell_level_niche_cluster_assign.loc[
                ana_data.meta_data_df[ana_data.meta_data_df["Sample"] == sample].index
            ].sum(axis=0)

        output_file_path = f"{ana_data.options.output}/{sample}_cluster_connectivity.pdf"

        plot_niche_cluster_connectivity(
            niche_cluster_connectivity=niche_cluster_connectivity,
            niche_cluster_score=ana_data.niche_cluster_score,
...
def plot_graph_niche_cluster_feat_similarity(
    ...
    if figsize_scale is None:
        figsize_scale = (
            1.0
            if niche_cluster_feat_similarity.shape[0] < 10
            else 1.5 * np.log10(niche_cluster_feat_similarity.shape[0])
        )
...
def plot_graph_niche_cluster_feat_similarity_from_anadata(
    ...
    output_file_path = (
        f"{ana_data.options.output}/niche_cluster_feature_similarity.pdf"
...
def plot_graph_niche_cluster_feat_similarity_bysample_from_anadata(ana_data: AnaData) -> None:
    ...
    for sample in ana_data.meta_data_df["Sample"].unique():
        ...
        niche_cluster_feat = cal_nc_feat(
            nc_assign=ana_data.niche_level_niche_cluster_assign.loc[
                ana_data.meta_data_df[ana_data.meta_data_df["Sample"] == sample].index
            ],
            ctc=ana_data.cell_type_composition.loc[
                ana_data.meta_data_df[ana_data.meta_data_df["Sample"] == sample].index
            ][ana_data.cell_type_codes["Cell_Type"].tolist()],
        )
        niche_cluster_feat_similarity = cal_nc_feat_similarity(niche_cluster_feat)
        ...
        plot_graph_niche_cluster_feat_similarity(
            niche_cluster_feat_similarity=niche_cluster_feat_similarity.values,
            niche_cluster_score=ana_data.niche_cluster_score,
            niche_cluster_size=niche_cluster_size,
            reverse=ana_data.options.reverse,
...
def plot_heatmap_niche_cluster_feat_similarity(
    ...
    sns.heatmap(
        niche_cluster_feat_similarity.loc[niche_cluster_order, niche_cluster_order],
        vmin=0,
        vmax=1,
        cmap="Reds",
        annot=True,
...
def plot_heatmap_niche_cluster_feat_similarity_from_anadata(ana_data: AnaData) -> Optional[Tuple[plt.Figure, plt.Axes]]:
    ...
    output_file_path = (
        f"{ana_data.options.output}/Heatmap_niche_cluster_feature_similarity.pdf"
        if ana_data.options.output is not None
        else None
    )

    return plot_heatmap_niche_cluster_feat_similarity(
        niche_cluster_feat_similarity=niche_cluster_feat_similarity,
...
def plot_heatmap_niche_cluster_feat_similarity_by_sample_from_anadata(ana_data: AnaData) -> None:
    ...
    for sample in ana_data.meta_data_df["Sample"].unique():
        ...
        plot_heatmap_niche_cluster_feat_similarity(
            niche_cluster_feat_similarity=niche_cluster_feat_similarity,
            niche_cluster_order=nc_order,
            output_file_path=output_file_path,
        )
...
def plot_cluster_proportion(
    niche_cluster_loading: pd.Series, niche_cluster_colors: List, output_file_path: Optional[Union[str, Path]] = None
) -> Optional[Tuple[plt.Figure, plt.Axes]]:
...
def plot_cluster_proportion_from_anadata(ana_data: AnaData) -> Optional[Tuple[plt.Figure, plt.Axes]]:
    ...
    return plot_cluster_proportion(
...
def plot_niche_cluster_loadings_dataset_from_anadata(
    ...
    return plot_niche_cluster_loadings_dataset(
        cell_level_niche_cluster_assign=ana_data.cell_level_niche_cluster_assign,
...
def plot_niche_cluster_loadings_sample_from_anadata(ana_data: AnaData) -> Optional[List[Tuple[plt.Figure, plt.Axes]]]:
    ...
    return plot_niche_cluster_loadings_sample(
        cell_level_niche_cluster_assign=ana_data.cell_level_niche_cluster_assign,
        meta_data_df=ana_data.meta_data_df,
        nc_scores=nc_scores,
...
def plot_max_niche_cluster_dataset_from_anadata(ana_data: AnaData) -> Optional[Tuple[plt.Figure, plt.Axes]]:
    ...
    return plot_max_niche_cluster_dataset(
        cell_level_max_niche_cluster=ana_data.cell_level_max_niche_cluster,
        meta_data_df=ana_data.meta_data_df,
        nc_scores=nc_scores,
        output_file_path=ana_data.options.output,
    )
...
def plot_max_niche_cluster_sample_from_anadata(ana_data: AnaData) -> Optional[List[Tuple[plt.Figure, plt.Axes]]]:
    ...
    return plot_max_niche_cluster_sample(
        cell_level_max_niche_cluster=ana_data.cell_level_max_niche_cluster,
        meta_data_df=ana_data.meta_data_df,
        nc_scores=nc_scores,
        output_file_path=ana_data.options.output,
...
def plot_niche_cluster_gini(
    intra_cluster_gini_df: pd.DataFrame, output_file_path: Optional[Union[str, Path]] = None
) -> Optional[Tuple[plt.Figure, plt.Axes]]:
...
def plot_niche_cluster_gini_from_anadata(ana_data: AnaData) -> Optional[Tuple[plt.Figure, plt.Axes]]:
    ...
    intra_cluster_gini_df = pd.DataFrame(
        data={"gini": intra_cluster_gini, "cluster": ana_data.cell_level_niche_cluster_assign.columns}
    )
    fig, ax = plt.subplots(figsize=(6, 4))
    sns.barplot(data=intra_cluster_gini_df, x="cluster", y="gini", ax=ax)
    ax.set_xlabel("Niche Cluster")
