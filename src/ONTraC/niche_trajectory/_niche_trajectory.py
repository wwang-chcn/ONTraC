"""Niche trajectory construction utilities for ONTraC."""
...
def trajectory_path_to_NC_score(niche_trajectory_path: List[int], niche_clustering_sum: ndarray) -> ndarray:
...
def get_niche_NTScore(
    trajectory_construct_method: str, niche_level_niche_cluster_assign_df: DataFrame, niche_adj_matrix: ndarray
    ...
    niche_trajectory_path = get_niche_trajectory_path(
        trajectory_construct_method=trajectory_construct_method, niche_adj_matrix=niche_adj_matrix
    )

    niche_clustering_sum = niche_level_niche_cluster_assign_df.values.sum(axis=0)
    niche_cluster_score = trajectory_path_to_NC_score(
        niche_trajectory_path=niche_trajectory_path, niche_clustering_sum=niche_clustering_sum
    )
    niche_level_NTScore_df = pd.DataFrame(
        niche_level_niche_cluster_assign_df.values @ niche_cluster_score,
...
def niche_to_cell_NTScore(meta_data_df: DataFrame, niche_level_NTScore_df: DataFrame, rel_params: Dict) -> DataFrame:
    ...
    sample_files_by_name_dict = {
        sample_files_dict["Name"]: sample_files_dict for sample_files_dict in rel_params["Data"]
    }
    sample_cell_level_NTScore_list = []

    for sample in samples:
        sample_niche_level_NTScore_df = niche_level_NTScore_df.loc[
            meta_data_df[meta_data_df["Sample"] == sample][id_name].values
        ]
        ...
        if niche_weight_matrix.shape[0] != sample_niche_level_NTScore_df.shape[0]:
            raise ValueError(
                f"Inconsistent number of niches in {sample} sample. "
        ...
        if niche_weight_matrix.shape[1] != sample_niche_level_NTScore_df.shape[0]:
            raise ValueError(
                f"Inconsistent number of cells in {sample} sample. "
                f"Please check the niche weight matrix and the niche-level NTScore."
            )
        ...
        sample_cell_level_NTScore_list.append(
            pd.DataFrame(
                cell_level_NTScore_.reshape(-1), index=sample_niche_level_NTScore_df.index, columns=["Cell_NTScore"]
            )
        )
...
def NTScore_table(
    ...
    niche_level_NTScore_df: DataFrame,
    cell_level_NTScore_df: DataFrame,
    rel_params: Dict,
) -> None:
    ...
    for sample in samples:
        ...
        sample_cell_level_NTScore_df = cell_level_NTScore_df.loc[
            meta_data_df[meta_data_df["Sample"] == sample][id_name].values
        ]
