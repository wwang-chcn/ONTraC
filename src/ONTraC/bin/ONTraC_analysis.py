...
def prepare_optparser() -> OptionParser:
    ...
    io_options = IO_OPTIONS['ONTraC_analysis']

    usage = """USAGE: %prog <--NN-dir PREPROCESSING_DIR> <--GNN-dir GNN_DIR>
...
def opt_validate(optparser: OptionParser) -> Values:
    ...
    validate_io_options(optparser=optparser, options=options, io_options=io_options, overwrite_validation=False)

    options.yaml = f'{options.NN_dir}/samples.yaml'
