"""Visualization argument parsing and validation for ONTraC."""
...
def add_visualization_group(optparser: OptionParser) -> None:
    ...
    group_vis.add_option(
        "-s", "--sample", dest="sample", action="store_true", default=False, help="Plot each sample separately."
    )
    group_vis.add_option(
        "--scale-factor",
        dest="scale_factor",
...
def add_suppress_group(optparser: OptionParser) -> None:
    ...
    group.add_option(
        ...
        default=False,
        help=(
            "Skip niche-cluster loadings visualization; useful when cluster count "
...
def validate_suppress_options(options: Values, optparser: Optional[OptionParser] = None) -> None:
    ...
    if getattr(options, "GNN_dir", None) is None:
        info(
            "GNN_dir is not set. suppress_niche_cluster_loadings is forced to True "
            "to skip niche cluster loading visualization."
        )
    ...
    if getattr(options, "NT_dir", None) is None:
        info(
            "NT_dir is not set. suppress_niche_trajectory is forced to True to skip "
            "niche trajectory visualization."
        )
