"""Training argument parsing and validation for ONTraC."""
...
def add_GCN_options_group(group_train: OptionGroup) -> None:
    ...
    # GCN options group
    group_train.add_option(
        "--hidden-feats", dest="hidden_feats", type="int", default=4, help="Number of hidden features. Default is 4."
    )
    group_train.add_option(
        "--n-gcn-layers", dest="n_gcn_layers", type="int", default=2, help="Number of GCN layers. Default is 2."
    )
...
def validate_GP_options(options: Values, optparser: Optional[OptionParser] = None) -> None:
    ...
    if getattr(options, "k", None) is None:
        ...
        warning(
            "We recommend k <= 8. Larger values may produce unstable results "
            "and increase niche trajectory runtime."
        )
