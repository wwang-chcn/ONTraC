"""I/O argument parsing and validation for ONTraC."""
    ...
    if ioc.has_io_option("input"):
        if ioc.get_io_option_attr("input") == "required":  # ONTraC & ONTraC_NN
            group_io.add_option(
                ...
                type="string",
                help=(
                    "Meta data CSV where each row is a cell. First column should be Cell_ID. "
                    "Columns x, y, and Sample are required."
                ),
            )
            group_io.add_option(
                "--exp-input",
                ...
                default=None,
                help=(
                    "Normalized expression CSV (#cell x #gene) with Cell_ID in the first column. "
                    "If omitted, Cell_Type must exist in meta data."
                ),
            )
            group_io.add_option(
                "--embedding-input",
            ...
            group_io.add_option(
                ...
                default=None,
                help=(
                    "Deconvoluted cell-type expression CSV. First column should match the "
                    "cell-type names used in deconvoluted_ct_composition."
                ),
            )
...
def validate_io_options(
    options: Values, io_options: Optional[Dict[str, List[str]]] = None, optparser: Optional[OptionParser] = None
) -> None:
    ...
    if ioc.has_io_option("NN_dir"):
        ...
        if ioc.get_io_option_attr("NN_dir") in ["required", "overwrite"] and getattr(options, "NN_dir", None) is None:
            ...
            sys.exit(1)
        # overwrite warning
        elif ioc.get_io_option_attr("NN_dir") == "overwrite" and (
            getattr(options, "NN_dir", None) is not None and os.path.isdir(options.NN_dir)
        ):
            warning(f"The directory ({options.NN_dir}) you given already exists. It will be overwritten.")
        # optional check
        elif ioc.get_io_option_attr("NN_dir") == "optional" and getattr(options, "NN_dir", None) is not None:
    ...
    if ioc.has_io_option("GNN_dir"):
        ...
        if ioc.get_io_option_attr("GNN_dir") in ["required", "overwrite"] and getattr(options, "GNN_dir", None) is None:
            ...
            sys.exit(1)
        # overwrite warning
        elif ioc.get_io_option_attr("GNN_dir") == "overwrite" and (
            getattr(options, "GNN_dir", None) is not None and os.path.isdir(options.GNN_dir)
        ):
            warning(f"The directory ({options.GNN_dir}) you given already exists. It will be overwritten.")
        # optional check
        elif ioc.get_io_option_attr("GNN_dir") == "optional" and getattr(options, "GNN_dir", None) is not None:
    ...
    if ioc.has_io_option("NT_dir"):
        ...
        if ioc.get_io_option_attr("NT_dir") in ["required", "overwrite"] and getattr(options, "NT_dir", None) is None:
            ...
            sys.exit(1)
        # overwrite warning
        elif ioc.get_io_option_attr("NT_dir") == "overwrite" and (
            getattr(options, "NT_dir", None) is not None and os.path.isdir(options.NT_dir)
        ):
            warning(f"The directory ({options.NT_dir}) you given already exists. It will be overwritten.")
        # optional check
        elif ioc.get_io_option_attr("NT_dir") == "optional" and getattr(options, "NT_dir", None) is not None:
    ...
    if ioc.has_io_option("input"):
        if ioc.get_io_option_attr("input") == "required":  # ONTraC & ONTraC_NN
            # meta data
            # HINT[deprecated]: legacy --dataset support
            ...
            if not os.path.isfile(options.meta_input):
                ...
                if optparser is not None:
                    optparser.print_help()
                sys.exit(1)
            # HINT[validation]: input format check
            ...
            if getattr(options, "deconvoluted_ct_composition", None) is None:  # set default None, for interactive mode
                ...
                if not os.path.isfile(options.deconvoluted_ct_composition):
                    error(
                        "Deconvoluted cell type composition file "
                        f"({options.deconvoluted_ct_composition}) does not exist."
                    )
                    if optparser is not None:
                        optparser.print_help()
                    sys.exit(1)
                if not options.deconvoluted_ct_composition.endswith(("csv", "csv.gz")):
                    error(
                        "Deconvoluted cell type composition file "
                        f"({options.decomposition_cell_type_composition_input}) must be csv/csv.gz."
                    )
                    if optparser is not None:
                        optparser.print_help()
                    sys.exit(1)
