...
def niche_network_construct(options: Values) -> None:
    ...
    deconvoluted_exp_input = (
        options.deconvoluted_exp_input
        if options.deconvoluted_exp_input is not None
        else Path(options.NN_dir).joinpath("celltype_x_gene_deconvolution.csv")
    )
...
def gnn(options: Values) -> None:
    ...
    loss_weight_args: Dict[str, float] = {
        key: value for key, value in options.__dict__.items() if key.endswith("loss_weight")
    }
    batch_train = train(
        nn_model=model,
    ...
    # predict
    consolidate_z_array, consolidate_s_array, _ = predict(
        output_dir=options.GNN_dir, batch_train=batch_train, dataset=dataset
    )
    # save results
    if consolidate_z_array is not None and consolidate_s_array is not None:
        save_graph_pooling_results(
            meta_data_df=meta_data_df,
            dataset=dataset,
            rel_params=get_rel_params(NN_dir=options.NN_dir, params=read_yaml_file(f"{options.NN_dir}/samples.yaml")),
...
def niche_trajectory_construct(options: Values) -> None:
    ...
    cell_level_NTScore_df = niche_to_cell_NTScore(
        meta_data_df=meta_data_df, niche_level_NTScore_df=niche_level_NTScore_df, rel_params=rel_params
    )

    NTScore_table(
        save_dir=options.NT_dir,
        meta_data_df=meta_data_df,
