...
def save_graph_pooling_results(meta_data_df: pd.DataFrame, dataset: SpatailOmicsDataset, rel_params: Dict,
    ...
    for i, data in enumerate(dataset):
        ...
        consolidate_z_df_['Cell_ID'] = meta_data_df[meta_data_df['Sample'] == data.name]['Cell_ID'].values
    ...
    # save results
    ## consolidate_z niche
    consolidate_z_niche_df = consolidate_z_niche_df.set_index('Cell_ID')
    consolidate_z_niche_df = consolidate_z_niche_df.loc[meta_data_df['Cell_ID'], :]
    consolidate_z_niche_df.to_csv(f'{output_dir}/niche_hidden_features.csv.gz', index=True, index_label='Cell_ID', header=True)
    ## consolidate_s niche
