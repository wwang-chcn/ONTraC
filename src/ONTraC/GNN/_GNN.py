...
def predict(
    ...
    for data in each_sample_loader:  # type: ignore
        ...
        for key, value in predict_result.items():
            np.savetxt(
                fname=f"{output_dir}/{data.name[0]}_{key}.csv.gz",
        ...
        if consolidate_flag:
            ...
            consolidate_out_adj: Tensor = (
                out_adj_ if consolidate_out_adj is None else consolidate_out_adj + out_adj_
            )  # type: ignore
            consolidate_out: Tensor = (
                out.squeeze(0) * data.mask.sum()  # type: ignore
                if consolidate_out is None
                else consolidate_out + out.squeeze(0) * data.mask.sum()
            )  # N x D
...
def save_graph_pooling_results(
    ...
    for i, data in enumerate(dataset):
        ...
        consolidate_z_df_ = pd.DataFrame(
            consolidate_z, columns=[f"HiddenFeat_{i}" for i in range(consolidate_z.shape[1])]
        )
        ...
        consolidate_s_df_ = pd.DataFrame(
            consolidate_s, columns=[f"NicheCluster_{i}" for i in range(consolidate_s.shape[1])]
        )
        ...
        niche_to_cell_matrix = (
            niche_weight_matrix / niche_weight_matrix.sum(axis=0)
        ).T  # normalize by the all niches associated with each cell, N x N

        consolidate_s_cell = niche_to_cell_matrix @ consolidate_s
        consolidate_s_cell_df_ = pd.DataFrame(
            consolidate_s_cell, columns=[f"NicheCluster_{i}" for i in range(consolidate_s_cell.shape[1])]
        )
    ...
    # save results
    # SECTION: Consolidated niche embeddings
    ...
    consolidate_z_niche_df.to_csv(
        f"{output_dir}/niche_hidden_features.csv.gz", index=True, index_label=id_name, header=True
    )
    # SECTION: Consolidated niche assignments
    ...
    consolidate_s_niche_df.to_csv(
        f"{output_dir}/niche_level_niche_cluster.csv.gz", index=True, index_label=id_name, header=True
    )
    consolidate_s_niche_df["Niche_Cluster"] = consolidate_s_niche_df.values.argmax(axis=1)
    consolidate_s_niche_df["Niche_Cluster"].to_csv(
        f"{output_dir}/niche_level_max_niche_cluster.csv.gz", index=True, header=True
    )
    ...
    consolidate_s_cell_df.to_csv(
        f"{output_dir}/cell_level_niche_cluster.csv.gz", index=True, index_label=id_name, header=True
    )
    consolidate_s_cell_df["Niche_Cluster"] = consolidate_s_cell_df.values.argmax(axis=1)
    consolidate_s_cell_df["Niche_Cluster"].to_csv(
        f"{output_dir}/cell_level_max_niche_cluster.csv.gz", index=True, header=True
    )
