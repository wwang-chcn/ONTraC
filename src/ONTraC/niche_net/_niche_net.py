"""Niche network construction utilities for ONTraC."""
...
def build_knn_network(
    sample_name: str, sample_meta_df: pd.DataFrame, n_neighbors: int = 50, n_local: int = 20
    ...
    n_local :
        int, index of distance used for normalization.

    Returns
    -------
    Tuple[np.ndarray, np.ndarray, np.ndarray] (coordinates, dis_matrix, indices_matrix)."""
...
def calc_edge_index(
    sample_name: str, sample_meta_df: pd.DataFrame, indices_matrix: np.ndarray, n_neighbors: int = 50
) -> np.ndarray:
    ...
    sample_meta_df :
        pd.DataFrame, sample data.
    indices_matrix :
        np.ndarray, indices matrix.
    n_neighbors :
        int, number of neighbors.

    Returns
    -------
    np.ndarray, edge index."""
    ...
    adj_matrix = csr_matrix(
        (np.ones(dst_indices.shape[0]), (src_indices, dst_indices)), shape=(N, N)
...
def gauss_dist_1d(dist: np.ndarray, n_local: int) -> float:
    ...
    Returns
    -------
    float, gaussian affinity."""
    return np.exp(-((dist / dist[n_local]) ** 2))


def calc_niche_weight_matrix(
    sample_name: str,
    ...
    Returns
    -------
    csr_matrix, niche weight matrix."""

    info(f"Calculating niche weight matrix for sample: {sample_name}...")

    N = sample_meta_df.shape[0]
    niche_weight_matrix = np.apply_along_axis(func1d=gauss_dist_1d, axis=1, arr=dis_matrix, n_local=n_local)[
        :, : n_neighbors + 1
    ]  # N x (k + 1), #niche x #cell
    ...
    niche_weight_matrix_csr = csr_matrix(
        (niche_weight_matrix.flatten(), (src_indices, dst_indices)), shape=(N, N)
    )  # convert to csr_matrix, N x N, #niche x #cell

    return niche_weight_matrix_csr


def calc_cell_type_composition(
    niche_weight_matrix: csr_matrix, ct_coding_matrix: Optional[np.ndarray] = None
) -> np.ndarray:
    ...
    niche_weight_matrix :
        csr_matrix, niche weight matrix.
    ct_coding_matrix :
        Optional[np.ndarray], decompsited cell type.
...
def save_niche_network(
    sample_name: str,
    sample_meta_df: pd.DataFrame,
    indices_matrix: np.ndarray,
    edge_index: np.ndarray,
    niche_weight_matrix: csr_matrix,
    cell_type_composition: np.ndarray,
...
def construct_niche_network_sample(
    sample_name: str,
    sample_meta_df: pd.DataFrame,
    ...
    sample_meta_df :
        pd.DataFrame, sample data.
    sample_ct_coding :
        pd.DataFrame, sample cell type coding.
    save_dir :
        str, save directory.
    n_neighbors :
        int, number of neighbors.
    ...
    Returns
    -------
    None. Saves coordinates/cell types, builds kNN graph, and writes edge index,
    weight matrix, and cell-type composition.
    """

    info(f"Constructing niche network for sample: {sample_name}.")

    # build kNN network
    coordinates, dis_matrix, indices_matrix = build_knn_network(
        sample_name=sample_name, sample_meta_df=sample_meta_df, n_neighbors=n_neighbors, n_local=n_local
    )

    # calculate edge index
    edge_index = calc_edge_index(
        sample_name=sample_name, sample_meta_df=sample_meta_df, indices_matrix=indices_matrix, n_neighbors=n_neighbors
    )

    # calculate niche_weight_matrix
    niche_weight_matrix = calc_niche_weight_matrix(
    ...
    cell_type_composition = calc_cell_type_composition(
        niche_weight_matrix=niche_weight_matrix, ct_coding_matrix=sample_ct_coding.values
    )
...
def construct_niche_network(
    ...
    for sample_name in samples:
        ...
        construct_niche_network_sample(
            sample_name=sample_name,
            sample_meta_df=sample_meta_df,
...
def ct_coding_adjust(
    ...
    if deconvoluted_exp_input is None:
        ...
        ct_embedding = (
            pd.concat(
                [
                    meta_data_df.set_index(meta_data_df.columns[0])["Cell_Type"],
                    embedding_df.iloc[:, 1:],
                ],  # remove Cell_ID
                axis=1,
            )
