...
def calc_cell_type_composition(options: Values, sample_data_df: pd.DataFrame, niche_weight_matrix: csr_matrix,
    ...
    # calculate cell type composition
    cell_to_niche_matrix = niche_weight_matrix / niche_weight_matrix.sum(axis=1)  # N (#niche) x N (#cell)
    if options.decomposition_cell_type_composition_input is None:
        one_hot_matrix = np.zeros(shape=(N,
                                         sample_data_df['Cell_Type'].cat.categories.shape[0]))  # N (#cell) x #cell_type
...
def construct_niche_network_sample(options: Values, sample_data_df: pd.DataFrame, sample_name: str) -> None:
    ...
    # calculate cell type composition
    cell_type_composition = calc_cell_type_composition(options=options,
                                                       sample_data_df=sample_data_df,
