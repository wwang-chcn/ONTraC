    ...
    if id_name == "Cell_ID":
        ...
        if low_res_exp_input is not None:
            ...
            if low_res_exp_df.columns.tolist() != ids:
                raise ValueError(
                    "The first row in low_res_exp_data should match the first column of meta_data.\n"
                    "low_res_exp_data must use #gene x #spot layout."
                )
        ...
        if (
            deconvoluted_exp_input is not None and deconvoluted_ct_composition is not None
        ):  # if deconvoluted_exp_input is provided, deconvoluted_ct_composition should also be provided
...
def perform_deconvolution(
    NN_dir: Union[str, Path], dc_method: str, exp_df: pd.DataFrame, dc_ct_num: int, gen_ct_embedding: bool = False
) -> pd.DataFrame:
    ...
    if dc_method == "STdeconvolve":
        ...
        ct_coding_df = apply_STdeconvolve(
            NN_dir=NN_dir, exp_df=exp_df, ct_num=dc_ct_num, gen_ct_embedding=gen_ct_embedding
        )
    ...
    if id_name == "Cell_ID":
        if "Cell_Type" in meta_data_df.columns:  # option 1: cell-level data with Cell_Type info in meta_data
            ...
            if gen_ct_embedding and input_data.get("embedding_data", None) is None:  # generate cell type embedding
                ...
                # save PCA embedding
                input_data["embedding_data"] = pd.DataFrame(
                    data=pca_embedding,
        ...
        ct_coding_matrix = np.zeros(
            shape=(meta_data_df.shape[0], meta_data_df["Cell_Type"].cat.categories.shape[0])
        )  # N x #cell_type
        ct_coding_matrix[np.arange(meta_data_df.shape[0]), meta_data_df.Cell_Type.cat.codes.values] = 1
        ct_coding_df = pd.DataFrame(
            data=ct_coding_matrix, columns=meta_data_df["Cell_Type"].cat.categories, index=meta_data_df[id_name]
        )
        # save cell type code
        ...
        if (
            "deconvoluted_ct_composition" in input_data
        ):  # option 4: low resolution data with deconvoluted cell type composition
            ...
            ct_coding_df = perform_deconvolution(
                NN_dir=NN_dir,
                dc_method=dc_method,
...
def preprocessing_nn(
    ...
    """Preprocessing for niche network.
        ...
        4. meta_input, NN_dir, low_res_exp_input, deconvoluted_ct_composition, deconvoluted_exp_input
           (spot-level data with deconvoluted cell type composition)
    ...
    input_data = cal_cell_type_coding(
