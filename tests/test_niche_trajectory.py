...
def test_get_niche_trajectory_path() -> None:
    ...
    actual_path = get_niche_trajectory_path(
        trajectory_construct_method=options.trajectory_construct,
        niche_adj_matrix=adj_matrix,
    )
    ...
    adj_matrix = np.array(
        [
            [0.0, 0.02272882, 0.26394865, 0.25664073, 0.00442298, 0.29683277],
            [0.02272882, 0.0, 0.00166663, 0.07405341, 0.57961249, 0.36278379],
            [0.26394859, 0.00166663, 0.0, 0.56053263, 0.00179466, 0.00084385],
            [0.25664064, 0.07405341, 0.56053275, 0.0, 0.09484559, 0.07408164],
            [0.00442298, 0.57961255, 0.00179466, 0.0948456, 0.0, 0.35368276],
            [0.29683274, 0.36278391, 0.00084385, 0.07408167, 0.35368285, 0.0],
        ]
    )
...
def test_trajectory_path_to_NC_score():
    ...
    expected_NT_score = np.array([1.0, 0.0, 0.2, 0.4, 0.6, 0.8])
    gen_NT_score = trajectory_path_to_NC_score(
        niche_trajectory_path=niche_trajectory_path,
        niche_clustering_sum=np.array([132.15, 481.15, 458.1, 35.94, 785.2, 53.157]),
    )
...
def test_get_niche_NTScore():
    ...
    niche_level_niche_cluster_assign_df = pd.DataFrame(
        np.array([[0.1, 0.2, 0.7], [0.4, 0.5, 0.1], [0.6, 0.1, 0.3]])
    )  # #niche x #niche_cluster
